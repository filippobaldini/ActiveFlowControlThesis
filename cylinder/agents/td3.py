...
class TD3(object):
    ...
    def select_action(self, state):
        ...
        actions = self.actor(state_tensor).detach().cpu().numpy()

        # Single env case
        if len(actions) == 1:
            return actions[0]  # shape: (3,)
        return actions  # shape: (num_envs, 3)
    ...
    def save(self, filename, directory):
        torch.save(
            self.actor.state_dict(), "%s/%sbest_actor.pth" % (directory, filename)
        )
        torch.save(
            self.critic.state_dict(), "%s/%sbest_critic.pth" % (directory, filename)
        )
    ...
    def load_all(self, filename, directory):
        if not torch.cuda.is_available():
            return torch.load(
                "%s/%s_all.pth" % (directory, filename), map_location="cpu"
            )
        else:
