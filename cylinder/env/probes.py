...
class Probe_Evaluator(object):
    ...
    def __init__(self, eval_func, probes_locations):
        ...
        for point_x, cell in zip(probes_locations, cells_list):
            ...
            if cell is not None:
                ...
                cell = Cell(mesh, cell)
                vertex_coords, orientation = (
                    cell.get_vertex_coordinates(),
                    cell.orientation(),
                )
                # Eval the basis function for the current cell through flattening the matrix in a vector by .ravel()
                basis_matrix.ravel()[:] = element.evaluate_basis_all(
                    point_x, vertex_coords, orientation
                )
    ...
    def sample(self, u):
        ...
        self.readings_local[:] = np.hstack(
            [f(function_vec) for f in self.probes]
        )  # Get local
...
class PressureProbeValues(Probe_Evaluator):
    ...
    def sample(self, u, p):
        return Probe_Evaluator.sample(self, p)
...
class VelocityProbeValues(Probe_Evaluator):
    ...
    def sample(self, u, p):
        return Probe_Evaluator.sample(self, u)
...
class TotalrecirculationArea(object):
    ...
    def __init__(self, velocity, threshold, include_cell=lambda x: True, store_path=""):
        ...
        cells_selected = filter(
            lambda cell_: include_cell(cell_.midpoint().array()), cells(mesh)
        )
        cells_selected, vol_cells_selected = list(
            zip(*((cell_.index(), cell_.volume()) for cell_ in cells_selected))
        )
    ...
    def sample(self, velocity, pressure):
        ...
        filtered_dofs = set(
            np.array(self.indices)[coefficient_filter]
        )  # Now global w.r.t to V
        # A cell with some dofs which are filtered, is counted as a cell where there is recirculation .
        self.recirculation_cells = [
            cell for cell, dofs in enumerate(self.cellsdof) if dofs & filtered_dofs
        ]

        recirc_area = sum(
            self.vol_cells_selected[cell] for cell in self.recirculation_cells
        )
