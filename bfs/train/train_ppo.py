...
flow_params = {
    "mu": 1e-3,
    "mu_list": [1e-3, 3.33e-4, 6.66e-4, 5e-4],
    "rho": 1,
...
run_name = f"BackwardFacingStep_ppo_{timestamp}"

if geometry_params["set_freq"]:
    run_name = f"BackwardFacingStep_no_freq_ppo_{timestamp}"
...
for iteration in range(1, EPISODES + 1):
    ...
    next_obs = torch.Tensor(next_obs).to(device)
    next_done = torch.zeros(1).to(device)
    print("Starting Episode:", iteration)
    ...
    print(f"Episode {iteration} done, Total Reward: {episode_reward}")
    ...
    if EARLY_STOPPING:
        ...
        if episode_reward > best_reward:
            ...
            # Save the best model
            model_path = os.path.join(model_dir, f"{ENV_NAME}_best_model.cleanrl_model")
            torch.save(agent.state_dict(), model_path)
            num_iterations_no_improve = 0  # Reset the counter

        else:
            num_iterations_no_improve += 1
            if num_iterations_no_improve >= PATIENCE:
                # Early stopping condition met
                print(
                    f"Early stopping at episode {iteration} with best reward {best_reward} at episode {best_episode}."
                )
                break
...
model_path = os.path.join(model_dir, f"training.cleanrl_model")
